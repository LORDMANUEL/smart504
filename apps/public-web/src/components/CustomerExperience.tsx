...
import { addClientVehicle, clientDocument, createClientAppointment, decideClientQuoteLine, getAppointmentSlots, getClientAppointments, getClientCompatibleParts, getClientDashboard, getClientRegistrationOptions, getMaintenancePackages, loginClient, logoutClient, redeemMaintenancePackage, registerClient, updateClientProfile } from '../lib/api';
import type { ClientRegistrationOptions } from '../lib/api';
import type { AppointmentSlot, ClientAppointment, ClientDashboard, MaintenancePackage, Product } from '../types';
...
export function CustomerPortal() {
  ...
  useEffect(() => {
    ...
    void getMaintenancePackages().then(setPackages).catch(() => setPackages([]));
  ...
  const nav = [['vehicle', CarFront, 'Mi vehículo'], ['appointments', CalendarDays, 'Agendar cita'], ['parts', PackageSearch, 'Repuestos compatibles'], ['benefits', Award, 'Puntos y paquetes'], ['alerts', AlertTriangle, 'Alertas'], ['quotes', FileCheck2, 'Cotizaciones'], ['invoices', FileText, 'Facturas']] as const;
  return <div className="client-portal">
    ...
    <main><header><div><p>Bienvenido, {dashboard.profile.full_name}</p><h1>{section === 'vehicle' ? selected.label : nav.find(([id]) => id === section)?.[2] ?? 'Configuración'}</h1></div><label>Vehículo<select aria-label="Vehículo" value={vehicleId} onChange={(event) => setVehicleId(event.target.value)}>{dashboard.vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.label}</option>)}</select></label></header>
      ...
      {section === 'benefits' && <section className="loyalty-packages"><header><div><h2>Paquetes de mantenimiento</h2><p>Canjee puntos por servicios para {selected.label}. Cada canje queda auditado y reservado para confirmación del taller.</p></div><strong><Award /> {dashboard.profile.loyalty_points} puntos</strong></header><div>{packages.map((item) => <article key={item.id}><Wrench /><span><small>{item.service}</small><h3>{item.name}</h3><p>{item.description}</p></span><div><b>{item.points.toLocaleString('es-HN')} puntos</b><button disabled={!item.available} onClick={async () => { setLoyaltyMessage(''); try { const result = await redeemMaintenancePackage(item.id, selected.id); setLoyaltyMessage(`${result.package_name} reservado. Saldo: ${result.remaining_points} puntos.`); await reloadDashboard(); setPackages(await getMaintenancePackages()); } catch (error) { setLoyaltyMessage(error instanceof Error ? error.message : 'No se pudo canjear.'); } }}>{item.available ? 'Canjear' : 'Puntos insuficientes'}</button></div></article>)}</div>{loyaltyMessage && <p role="status">{loyaltyMessage}</p>}</section>}
