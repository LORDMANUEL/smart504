...
export type MarketingCampaign = { id: string; title: string; description: string; audience: string; valid_from: string | null; valid_until: string | null; price_from: number | null; call_to_action: string; tv_enabled: boolean; display_seconds: number; slug: string; status: 'DRAFT' | 'PUBLISHED'; media_url: string | null; media_type: 'IMAGE' | 'VIDEO' | null; clicks: number; public_path: string; created_at: string };
