...
import { createCampaign, createMaintenancePackage, getAdminDocument, getCampaigns, getMaintenancePackages, getPublicCampaigns, publishCampaign, uploadCampaignMedia } from '../lib/api';
import type { FlowHeatmapCell, MaintenancePackage, ManagementSummary, MarketingCampaign, OperationsOverview, StaffUser, WorkOrderCard, WorkOrderPartRequest } from '../types';
...
export function MarketingView({ token }: { token: string }) {
  const initialDraft = { title: '', description: '', audience: 'Todos los vehículos', valid_from: '', valid_until: '', price_from: 0, call_to_action: 'Agenda hoy', tv_enabled: true, display_seconds: 12, promo_code: '', discount_percent: 0, store_banner: true };
  const [items, setItems] = useState<MarketingCampaign[]>([]);
  const [draft, setDraft] = useState(initialDraft);
  const [error, setError] = useState('');
  const [packages, setPackages] = useState<MaintenancePackage[]>([]);
  const [packageDraft, setPackageDraft] = useState({ name: '', description: '', points: 1000, service: 'Mantenimiento preventivo' });
  useEffect(() => { void getCampaigns(token).then(setItems).catch((cause: Error) => setError(cause.message)); void getMaintenancePackages(token).then(setPackages).catch((cause: Error) => setError(cause.message)); }, [token]);
  async function add(event: React.FormEvent) {
    event.preventDefault();
    try {
      const created = await createCampaign(token, { ...draft, price_from: draft.price_from || undefined, promo_code: draft.promo_code || undefined });
  ...
  async function addPackage(event: React.FormEvent) { event.preventDefault(); try { const created = await createMaintenancePackage(token, packageDraft); setPackages((current) => [...current, created]); setPackageDraft({ name: '', description: '', points: 1000, service: 'Mantenimiento preventivo' }); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo crear el paquete.'); } }
  return <div className="role-view">
    ...
    <div className="marketing-workspace"><form className="role-panel campaign-create" onSubmit={add}>
      ...
      <label>Código de descuento<input maxLength={40} value={draft.promo_code} onChange={(event) => setDraft({ ...draft, promo_code: event.target.value.toUpperCase() })} placeholder="PATRIA504" /></label>
      <label>Descuento porcentual<input type="number" min="0" max="80" value={draft.discount_percent} onChange={(event) => setDraft({ ...draft, discount_percent: Number(event.target.value) })} /></label>
      ...
      <div><label><input type="checkbox" checked={draft.tv_enabled} onChange={(event) => setDraft({ ...draft, tv_enabled: event.target.checked })} /> Mostrar en TV</label><label><input type="checkbox" checked={draft.store_banner} onChange={(event) => setDraft({ ...draft, store_banner: event.target.checked })} /> Banner en tienda</label><label>Segundos<input type="number" min="5" max="120" value={draft.display_seconds} onChange={(event) => setDraft({ ...draft, display_seconds: Number(event.target.value) })} /></label></div>
      <button className="role-primary"><Megaphone /> Crear borrador</button>
    </form><section className="campaign-board">{items.map((item) => <article className="role-panel campaign-card" key={item.id}><div className="campaign-media">{item.media_url ? item.media_type === 'VIDEO' ? <video src={item.media_url} controls /> : <img src={item.media_url} alt={item.title} /> : <span><Image /><Video /><small>Sin archivo</small></span>}</div><div className="campaign-card-body"><header><span><small>{item.status} · {item.tv_enabled ? 'TV activa' : 'Sólo enlace'}</small><h2>{item.title}</h2></span><b><MousePointerClick /> {item.clicks} clics</b></header><p>{item.description}</p><small>{item.audience} · {item.display_seconds}s{item.promo_code ? ` · ${item.promo_code} (${item.discount_percent ?? 0}%)` : ''}</small><div className="campaign-link"><Link2 /><code>{window.location.origin}{item.public_path}</code><button aria-label={`Copiar enlace ${item.title}`} onClick={() => void navigator.clipboard.writeText(`${window.location.origin}${item.public_path}`)}><Copy /></button></div><footer><label className="campaign-upload"><Upload /> Cargar imagen o video<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" onChange={(event) => void upload(item, event.target.files?.[0])} /></label><button className={item.status === 'PUBLISHED' ? 'campaign-live' : ''} disabled={item.status === 'PUBLISHED'} onClick={() => void publish(item)}>{item.status === 'PUBLISHED' ? 'Publicada' : 'Publicar'}</button></footer></div></article>)}{!items.length && <div className="disabled-feature"><Megaphone /><h2>Sin campañas</h2><p>Cree la primera campaña con el formulario.</p></div>}</section></div>
    <section className="role-panel maintenance-package-admin"><header><div><h2>Paquetes canjeables</h2><p>Defina servicios que los clientes pueden reservar usando sus puntos.</p></div><b>{packages.length} configurados</b></header><form onSubmit={addPackage}><label>Nombre<input required value={packageDraft.name} onChange={(event) => setPackageDraft({ ...packageDraft, name: event.target.value })} /></label><label>Servicio<input required value={packageDraft.service} onChange={(event) => setPackageDraft({ ...packageDraft, service: event.target.value })} /></label><label>Puntos<input required type="number" min="1" value={packageDraft.points} onChange={(event) => setPackageDraft({ ...packageDraft, points: Number(event.target.value) })} /></label><label>Descripción<input required value={packageDraft.description} onChange={(event) => setPackageDraft({ ...packageDraft, description: event.target.value })} /></label><button className="role-primary"><Plus /> Crear paquete</button></form><div>{packages.map((item) => <article key={item.id}><Wrench /><span><strong>{item.name}</strong><small>{item.service} · {item.points.toLocaleString('es-HN')} puntos</small></span></article>)}</div></section>
  </div>;
}
