...
class ClientUser(SQLAlchemyBaseUserTableUUID, TimestampMixin, Base):
    ...
    managed_email: Mapped[str | None] = mapped_column(String(320), unique=True, index=True)
    mailbox_status: Mapped[str] = mapped_column(String(30), default="PENDING_CONFIGURATION", nullable=False)
    failed_login_attempts: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    locked_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    mfa_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    mfa_secret_encrypted: Mapped[str | None] = mapped_column(String(500))
